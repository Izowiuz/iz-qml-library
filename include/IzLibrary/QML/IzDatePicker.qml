...
Popup {
	...
	property date selectedDate: root.currentDate
	...
	onAboutToShow: {
		if (root.sanitizeDate(root.from) > root.sanitizeDate(root.to)
				|| root.sanitizeDate(root.to) < root.sanitizeDate(root.from)) {
			root.from = root.to;
			root.isValid = false;
		}
		if (root.sanitizeDate(root.currentDate) > root.sanitizeDate(root.to)
				|| root.sanitizeDate(root.currentDate) < root.sanitizeDate(root.from)) {
			root.from = root.to;
	...
	contentItem: ColumnLayout {
		...
		MonthGrid {
			...
			delegate: IzText {
				...
				font.bold: root.compareDates(root.selectedDate, model.date)
				opacity: model.month === monthGrid.month
						 //? ( model.date > root.sanitizeDate(root.to) || model.date <= root.sanitizeDate(root.from) ? 0.23 : 1 )
						 ? ( root.sanitizeDate(model.date) <= root.sanitizeDate(root.to) && root.sanitizeDate(model.date) >= root.sanitizeDate(root.from) ? 1 : 0.23 )
						 : 0

				Rectangle {
					anchors.fill: parent
					color: root.compareDates(root.selectedDate, model.date) ? root.Material.accent : root.Material.foreground
					opacity: 0.3
				}
			}
			...
			onClicked: {
				...
				root.selectedDate = root.sanitizeDate(date);
				if (root.compareDates(monthGrid.lastSelectedDate, root.selectedDate)) {
					root.dateSelected(root.selectedDate);
					root.close();
				} else {
					monthGrid.lastSelectedDate = root.selectedDate;
				}
			}
		}
		...
		RowLayout {
			...
			IzText {
				...
				text: Qt.formatDateTime(root.selectedDate, "dd-MM-yyyy")
			}
